
<div align="center">

# 🕘 996 Agent - The Tech Company

**1 CTO + 7 Specialists. AI works 996, so you don't have to.**

[![GitHub Stars](https://img.shields.io/github/stars/badhope/996-Skill?style=for-the-badge)](https://github.com/badhope/996-Skill/stargazers)
[![GitHub Forks](https://img.shields.io/github/forks/badhope/996-Skill?style=for-the-badge)](https://github.com/badhope/996-Skill/network/members)
[![GitHub Issues](https://img.shields.io/github/issues/badhope/996-Skill?style=for-the-badge)](https://github.com/badhope/996-Skill/issues)
[![GitHub License](https://img.shields.io/github/license/badhope/996-Skill?style=for-the-badge)](LICENSE)
[![CI Status](https://img.shields.io/github/actions/workflow/status/badhope/996-Skill/validate.yml?style=for-the-badge&label=CI)](.github/workflows/validate.yml)

[![Trae Skill](https://img.shields.io/badge/Trae-Skill-blue?style=for-the-badge&logo=ai)](https://trae.ai)
[![Made with AI](https://img.shields.io/badge/Made%20with-AI-red?style=for-the-badge)](https://github.com/topics/ai)
[![Multi-Agent](https://img.shields.io/badge/Multi-Agent-purple?style=for-the-badge)](https://github.com/topics/multi-agent)
[![Production Ready](https://img.shields.io/badge/Production-Ready-green?style=for-the-badge)]()

---

**🌍 [English](README_EN.md) | [中文](README.md)**

> **World's first multi-agent competitive production system. Simulates a complete 8-person tech company, delivering production-grade code quality through involution mechanisms.**

---

</div>

## ✨ Core Features

### 🎭 Real Company Organization
- **👨‍💼 1 CTO** - Brutal full-stack technical reviewer, ultimate veto power
- **👷 7 Specialists** - PM/Efficiency/UX/Architecture/Innovation/QA/Documentation
- **🔫 Elimination System** - Fire the worst performer every round
- **📊 Cross Review Matrix** - 7×7 = 49 peer reviews

### ⚡ Proven Quality Improvement

| Task Type | Regular AI | 996 Agent | Improvement |
|-----------|------------|-----------|-------------|
| Simple Task | 81 pts | 93 pts | +15% |
| Medium Task | 67 pts | 89 pts | +33% |
| Complex Task | 43 pts | 95 pts | **+121%** |
| **Average** | **62 pts** | **90.8 pts** | **+46%** |

> 📊 **Data Source**: Blind controlled experiment across 5 difficulty levels. See [BENCHMARK_REPORT.md](BENCHMARK_REPORT.md)

---

## 🚀 Quick Start

In Trae IDE, just type:
```
@996-agent write a production-grade XXX
```

That's it. The rest is handled automatically by your AI company.

---

## 📚 Documentation

| Document | Content |
|----------|---------|
| [Architecture Evolution](docs/ARCHITECTURE_EVOLUTION.md) | 3 generations from meme to industrial system |
| [Benchmark Report](BENCHMARK_REPORT.md) | Complete data from 5 difficulty levels |
| [Infinite Involution](docs/INFINITE_INVOLUTION.md) | Iteration marginal utility + SOP |
| [Open Source Viral Guide](docs/OPEN_SOURCE_VIRAL_GUIDE.md) | Complete playbook from 0 to 1000⭐ |
| [Skill Source](.trae/skills/996-agent/SKILL.md) | Full Skill implementation |

---

## 🏗️ V3.0 Team Structure

| Role | Specialization | Core Value Proposition |
|------|---------------|-----------------------|
| 👨‍💼 **CTO** | Full-stack Technical Review | Find fatal flaws everyone else misses |
| 📋 **Project Manager** | Process Methodology | No process = No consistency |
| ⚡ **Efficiency Expert** | Algorithm Optimization | Nobody else cares about performance |
| 🎨 **UX Designer** | API Ergonomics | Devs write unusable APIs |
| 🏗️ **Architect** | Enterprise Patterns | Everyone else writes toy code |
| 🧪 **Innovation Researcher** | Cross-domain Ideas | If everyone does it, it's not innovation |
| 🐞 **QA Engineer** | Destructive Testing | Authors don't break their own code |
| 📝 **Technical Writer** | Information Architecture | Good devs suck at documentation |

---

## 📜 Standard Workflow

1. **CTO Kickoff** - "The worst performer stays late today"
2. **Blind Development** - 7 people work independently
3. **7×7 Cross Review** - Everyone reviews everyone
4. **Competitive Iteration** - Must adopt 3 ideas from competitors
5. **CTO Brutal Review** - Point out flaws one by one
6. **Elimination & Delivery** - Fire the worst, ship the champion

---

## 💡 Key Research Findings

**It's not "three cobblers equal Zhuge Liang". It's:**

1. **Specialization** - Each person only needs to excel at one thing
2. **Perspective Diversity** - 7 completely different angles on the same problem
3. **Blindspot Cancellation** - Your competitor sees bugs you don't
4. **Competitive Pressure** - Real fear of elimination drives learning
5. **Ultimate Referee** - Solves "who reviews the reviewers" problem

> 🎯 **Empirical Law**: +7% quality per specialized role
> **Sweet Spot**: 8 people. Beyond that communication costs exceed benefits.

---

## ♾️ Infinite Involution Mode

User explicitly triggers with:
```
"Keep involuting"
"3 more rounds"
"Involution until satisfied"
"Fire the worst one"
```

Four intensity levels + Innovation entropy monitoring to prevent busywork.

---

## 🤝 Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md) for details. All contributions welcome!

## 📄 License

MIT - See [LICENSE](LICENSE) for details.

---

<div align="center">

**⭐ If this project made you laugh or think, give it a star!**

**AI works 996, so you don't have to.**

---

[![Stargazers over time](https://starchart.cc/badhope/996-Skill.svg?variant=adaptive)](https://starchart.cc/badhope/996-Skill)

</div>
